defmodule SymphonyElixir.CoreTest do
  ...
  test "in-repo WORKFLOW.md renders correctly" do
    ...
    assert prompt =~ "Leave problem breadcrumbs without comment spam"
    assert prompt =~ "Update the single `## Codex Workpad` for transient failures"
    assert prompt =~ "Add a separate concise Linear problem comment"
    assert prompt =~ "command/subsystem involved"
    assert prompt =~ "`Blocked` -> true external blocker recorded"
    assert prompt =~ "Blocked state missing"
